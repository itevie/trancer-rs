...
command_file!(TrancerCommand::<ViewTriggersArgs> {
    name: "usabletriggers".to_string(),
    t: TrancerCommandType::Hypnosis,
    description: "View yours or someone else's usable triggers (depending on traffic light)"
        .to_string(),
    ...
    handler: trancer_handler!(|ctx, args| {
        let triggers = UserImposition::fetch_all_for_by_tag(
            &ctx.sy,
            args.user.id,
            &vec![ImpositionTag::from(ctx.user_data.hypno_status.clone())],
        )
        .await?;
